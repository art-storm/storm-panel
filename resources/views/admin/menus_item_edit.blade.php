            ...
            <div class="row mb-2">
                <div class="col-sm-12">
                    <h1 class="m-0 text-dark">
                        Edit Menu Item for menu <span class="bg-info">'{{ $menu->name }}'</span>
                    </h1>
                </div><!-- /.col -->
            </div><!-- /.row -->
        </div><!-- /.container-fluid -->
        ...
        <div class="container-fluid">
            ...
            <div class="card">
                <div class="card-body">
                    ...
                    <form method="POST" action="{{ route('admin.menuitems.update', ['item_id' => $item->id]) }}">
                        @method('PUT')
                        @csrf
                        ...
                        <div class="form-group text-muted col-md-6">
                            ...
                            <input id="icon_class" type="text" class="form-control @error('icon_class') is-invalid @enderror"
                                   name="icon_class" value="{{ old('icon_class', $item->icon_class) }}">
                        ...
                        <div class="form-group text-muted col-md-6">
                            ...
                            <div class="input-group input-colorpicker">
                                <input type="color" class="form-control" name="color" value="{{ old('color', $item->color) }}">
                            </div>
                        </div>

                        <div class="form-group text-muted col-md-6 mt-4">
                            <label><input type="checkbox" name="divider" value="1"
                                          @if(old('divider', $item->divider) == '1') checked @endif>
                        ...
                        <div class="form-group text-muted col-md-6">
                            ...
                            <select name="target" class="form-control">
                                @foreach ($array_target as $key => $value)
                                    <option value="{{ $key }}" {{ ( $key == old('target', $item->target) ) ? 'selected' : '' }}>{{ $value }}</option>
                                @endforeach
                            </select>
                        </div>
